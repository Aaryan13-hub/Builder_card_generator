...
function updateGenerateEnabled() {
  generateBtn.disabled = !(uploadBlob && nameInput.value.trim() && roleInput.value.trim());
}
...
function drawScaledToCanvas(img, canvas) {
  const scale = Math.min(1, DETECTION_MAX_DIM / Math.max(img.naturalWidth, img.naturalHeight));
...
photoInput.addEventListener('change', async () => {
  ...
  try {
    ...
    if (await isHeicFile(file)) {
      ...
      const converted = await HeicTo.heicTo({ blob: file, type: 'image/jpeg', quality: 0.92 });
    ...
    if (modelsReady) {
      ...
      if (detection) {
        ...
        detectionStatus.textContent = 'Face detected — this area will be centered in your card.';
      } else {
        faceBoxFraction = null;
        detectionStatus.textContent = "Couldn't auto-detect a face — we'll auto-center your photo instead.";
      }
    } else {
      faceBoxFraction = null;
      detectionStatus.textContent = "Face detection unavailable — we'll auto-center your photo instead.";
    }
  } catch (err) {
    console.error(err);
    uploadBlob = null;
    showError('Could not read that photo. Try a different file (JPG, PNG, or HEIC).');
...
generateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(data.error || 'Something went wrong generating your card.');
    }
  ...
  try {
    if (navigator.canShare) {
      ...
      if (navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], text: shareText, url: sharePageUrl });
        return;
      }
    }
  } catch (err) {
